...
const echoStdin = "process.stdin.on('data', b => process.stdout.write(b))";
const upperCaseStdin = "process.stdin.on('data', b => process.stdout.write(b.toString().toUpperCase()))";
const countingJsonlServer = "let count=0; require('node:readline').createInterface({input:process.stdin})"
  + ".on('line', s => console.log(JSON.stringify({request:JSON.parse(s),count:++count})))";

/** Runs body with a fresh Shell and always closes it, even when an assertion fails. */
type ShellOptions = ConstructorParameters<typeof Shell>[0];
async function withShell(body: (shell: Shell) => Promise<void>, options?: ShellOptions) {
...
test('incremental UTF-8 framing, monitor deduplication/coalescing and truncation', () => {
  ...
  // Split inside the four-byte emoji so the decoder must carry state across chunks.
  framer.append(utf.subarray(0, 2));
  framer.append(utf.subarray(2));
  framer.close();
  ...
  assert.equal(data.lines.length, 8);
  assert.equal(data.omitted, 5);
...
test('argv is literal, stdin works, nonzero and missing executable are receipts', async () => {
  await withShell(async shell => {
    const literalArgs = ['-e', 'console.log(process.argv[1])', '$(must-not-execute)'];
    const literal = await shell.exec({ command: process.execPath, args: literalArgs });
    assert.equal(literal.stdout.trim(), '$(must-not-execute)');
    const input = await shell.exec({ ...node(echoStdin), input: 'hello\n' });
    assert.equal(input.stdout, 'hello\n');
    const fail = await shell.exec(node('process.exit(7)'));
...
test('shell heredocs/pipelines and native process piping compose', async () => {
  await withShell(async shell => {
    ...
    const consumer = shell.spawn(node(upperCaseStdin));
    producer.pipeTo(consumer);
    producer.end();
    const [produced, consumed] = await Promise.all([producer.wait(), consumer.wait()]);
    assert.equal(produced.exitCode, 0);
    assert.equal(consumed.stdout, 'PIPELINE');
  });
});

test('persistent JSONL subprocess retains state across requests', async () => {
  await withShell(async shell => {
    const job = shell.spawn(node(countingJsonlServer));
    const replies = job.lines();
    const nextCount = async () => JSON.parse((await replies.next()).value!).count;
    await job.write('{"id":1}\n');
    assert.equal(await nextCount(), 1);
    await job.write('{"id":2}\n');
    assert.equal(await nextCount(), 2);
    job.end();
    assert.equal((await job.wait()).state, 'exited');
...
test('output tails and protocol lines are bounded independently', async () => {
  await withShell(async shell => {
    ...
    await rejection;
    assert.ok(result.truncated.stdout);
...
test('observer publication failures stop owned work instead of leaking it', async () => {
  ...
  await withShell(async shell => {
    const job = shell.spawn(node(idle));
    const result = await job.wait();
    assert.equal(result.state, 'failed');
    assert.match(result.error!, /observer failed/);
  }, { events });
  const failingRegistration = () => { throw new Error('fixture registration failed'); };
  await withShell(async registration => {
    assert.equal((await registration.exec(node(idle))).state, 'failed');
  }, { onProcess: failingRegistration });
});
...
test('program completion cleans up outstanding jobs; handoff is not success', async () => {
  ...
  const result = await runProgram(defineProgram(async context => {
    job = context.shell.spawn(node(idle));
    context.emit('ready', { ok: true });
    return { value: 42 };
  }));
  assert.equal(result.state, 'completed');
  assert.equal((await job!.wait()).state, 'cancelled');
