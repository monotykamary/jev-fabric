...
for name in core io fail-fast jobs jobs-streams codec codec-limits wire codec-wire-probe http jev-client session monitor monitor-pure policy-core time-core timers; do
  printf '\nBuilding native fixture: %s\n' "$name"
  build/jev-fabric -- exec 300000 bend "native/tests/$name.bend" -o "build/test-$name"
done
build/test-core
